                ...
                <div class="card-body">
                    <form action="{{ route('admin.categories.store') }}" method="post" class="instant_reload_form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="name" value="{{ $category->name }}" class="form-control" placeholder="{{ __('Enter category name') }}" required>
                        </div>

                        <div class="form-group">
                            <label for="status" class="required">{{ __('Status') }}</label>
                            <select name="status" id="status" data-control="select2" required>
                                <option value="1" @selected($category->status)>{{ __("Active") }}</option>
                                <option value="0" @selected(!$category->status)>{{ __("Inactive") }}</option>
                            </select>
                        </div>
